...
sealed class JsonField<out T : Any> {
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing ->
                throw StrezlessMusickNexusMetadataInvalidDataException("`$name` is not set")
            is JsonNull -> throw StrezlessMusickNexusMetadataInvalidDataException("`$name` is null")
            else ->
                throw StrezlessMusickNexusMetadataInvalidDataException(
